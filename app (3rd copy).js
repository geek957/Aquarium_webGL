...
var final_textures=[];
var final_indices=[];
var loadfunc = function(objname,imagename,callback){
...
var InitDemo = function () {
	loadfunc('./Shark.json','./greatwhiteshark.png',function(err){
...
var RunDemo = function (vertexShaderText, fragmentShaderText, SusanImage, SusanModel) {
	...
	var susanVertices = SusanModel.meshes[2].vertices ;
	...
	var susanIndices = [].concat.apply([], SusanModel.meshes[2].faces);
	...
	var susanTexCoords = SusanModel.meshes[2].texturecoords[0];
...
//}
	...
	final_textures[0]=susanTexture;
	final_indices[0]=susanIndices;
};
...
var main=function()
{
	...
	var x_pos=0.0,y_pos=0.0,z_pos=0.0;
	//
	// Main render loop
	//
	...
	var loop = function () {
		...
		var r=(Math.random() * (0.1 - 0.0000)).toFixed(10);
		//console.log(r);
		...
		y_pos=parseFloat(y_pos)+parseFloat(y);
		z_pos=parseFloat(z_pos)+parseFloat(z);
		console.log(x_pos+"   "+y_pos+"  "+z_pos);
		//translate
		//var translation;
		//vec3.set (translation, 3, 4, 0.0);
		mat4.translate (mvMatrix, identityMatrix, [x_pos,y_pos,z_pos]);
